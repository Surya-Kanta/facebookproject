        ...
        <div id="wrapper">
            ...
            <div id="page-wrapper" >
                <div id="page-inner">
                    <div class="row">
                        <div class="col-md-12">
                            <h2><?php if(isset($_SESSION['accessToken'])) {echo $_SESSION['name'] . ', your Photos are here...'; } else {echo "Login to Continue"; } ?></h2>  
                        </div>
                    </div>              
                    <!-- /. ROW  -->
                    <hr />
                    ...
                    if(isset($_POST['chkdPost'])) {
                       ...
                       // foreach($_POST['chkdPost'] as $key1) {
                            ...
                            echo "Successfully deleted ".count($_POST['chkdPost']). " photos from Facebook";
                     //   }
                    }
                    ...
                    try {
                        ...
                        $total_photos = array_merge($total_photos, $photos_array);
...
//                      
                    ...
                    <form method='post'>
                        ...
                        <div style="float: right; margin:5px;align-content: center; justify-content: flex-end" class="btn-toolbar">
                            <div class="btn-group">
                        ...
                        <div style="width: 70%">
                        <!--<input class="btn-success btn-3" style="border-radius:10px;" type="submit" value="Execute Selected" onclick="alert('Are you sure??');"/><hr />-->
                        <?php
                        try {
                            foreach ($total_photos as $key) {
                                $p_req = $fb -> get('/'.$key['id'].'?fields=created_time,name,link,images');
                                $photo = $p_req -> getGraphNode() -> asArray();

                                echo '<input type="checkbox" name="chkdPost" value="'.$key['id'].'"><strong>Post ID: ' . $key['id'] . '</strong><br />';
                                echo $key['created_time']->format('d/M/Y - h:m:s')."<br />";
                                if(isset($key['name'])) { echo $key['name'] . '<br />'; }
                                
                                echo '<div class="fb-post" 
                                    data-href="'. $photo['link']. '" data-width="350" data-show-text="true">
                                    </div>';
                                echo '<div class="fb-comments" 
                                            data-href="'. $photo['link']. '" data-width="50" data-numposts="3">
                                </div>';
                                echo '<hr />';
